...
int main(void)
{
	...
	//CRITIC SECTION
	//Copiando en las matrices los valores almacenados en la memoria compartida 
	k = 0;
	for(i = 0; i < SIZE; i++)
		for(j = 0; j < SIZE; j++)
	...
	//Esperando la finalizacion del nieto, es necesario para no dejar al semaforo 1 abandonado
	WaitForSingleObject(piChild.hProcess, INFINITE);

	//Liberando Semaforo 1
